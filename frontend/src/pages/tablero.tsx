  ...
  return (
    <div ref={containerRef} className="relative flex items-center gap-2">
      ...
      {value && (
        <button
          type="button"
          onClick={() => { onChange(null); setInputText('') }}
...
function TareaCard({
  ...
  return (
    <div className={cn(
      'rounded-xl border p-3 space-y-2 bg-white dark:bg-zinc-900/80 transition-all group',
      vencida
          ...
          >
            ...
            <span className="truncate">
              {tarea.expediente.caratula
                ? tarea.expediente.caratula.length > 22
                  ? tarea.expediente.caratula.slice(0, 22) + '…'
                  : tarea.expediente.caratula
...
export default function TableroPage() {
  ...
  const asignadoFilter = misTareas
    ? (profile?.id ?? null)
    : (filtroAsignado || null)
  ...
  async function handleComplete(tarea: TareaWithRelations) {
  ...
  async function handleMoveToProgress(tareaId: string) {
    try {
      await updateTarea.mutateAsync({ id: tareaId, estado: 'EN_PROGRESO' })
    } catch {
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-zinc-200 dark:border-white/10 overflow-hidden text-xs font-medium">
          ...
          <button
            ...
            className={cn(
              'px-3 py-1.5 transition-colors border-l border-zinc-200 dark:border-white/10',
              !misTareas
                ? 'bg-blue-600 text-white'
                : 'bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-white/5',
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[0, 1, 2].map(i => (
            <div key={i} className="space-y-2">
              ...
              {[0, 1, 2].map(j => (
                <div key={j} className="h-20 bg-zinc-100 dark:bg-zinc-800 rounded-xl animate-pulse" />
              ))}
            </div>
          ))}
        </div>
            ...
            {pendientes.length === 0
              ...
              : pendientes.map(t => (
                <TareaCard
                  ...
                  onMoveToProgress={() => handleMoveToProgress(t.id)}
                />
              ))}
          </Column>
