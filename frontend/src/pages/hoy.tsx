...
import {
  ...
  AlertTriangle, Loader2, Upload, Clock, FolderOpen, FileText,
} from 'lucide-react'
...
export default function HoyPage() {
  const { data, isLoading, isError } = useHoy()
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatChip label="Audiencias hoy" value={data.audiencias_hoy.length} icon={Calendar} color="cyan" />
        <StatChip label="Tareas para hoy" value={data.tareas_hoy_count} icon={CheckSquare} color="emerald" />
        <StatChip label="Tareas vencidas" value={data.tareas_vencidas_count} icon={AlertTriangle} color={data.tareas_vencidas_count > 0 ? 'rose' : 'muted'} />
        <StatChip label="Consultas últ. 7d" value={data.consultas_nuevas_count} icon={Users} color="amber" />
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-zinc-900/30 p-3">
        ...
        <div className="flex flex-wrap gap-2">
          <QuickAction to="/clientes/nuevo" icon={Plus} label="Nueva consulta" color="emerald" />
          <QuickAction to="/agenda" icon={Calendar} label="Agendar turno" color="cyan" />
          <QuickAction to="/expedientes" icon={Upload} label="Subir documento" color="violet" />
...
function StatChip({ label, value, icon: Icon, color }: {
  ...
  return (
    <div className={cn('rounded-xl border px-4 py-3', colors)}>
    ...
    <div className="rounded-xl border border-white/10 bg-zinc-900/30 p-4">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((a) => (
            <Link key={a.id} to={`/expedientes/${a.expediente_id}`} className="block rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2 hover:bg-white/[0.04] transition-colors">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="shrink-0 inline-flex items-center gap-1 rounded-full bg-cyan-500/15 px-2 py-0.5 text-[11px] font-medium text-cyan-300 tabular-nums">
