...
function useExpedienteSearch(term: string) {
  ...
  return useQuery({
    queryKey: ['tareas-exp-search', term],
    queryFn: async () => {
...
const PRIORIDAD_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  BAJA:    { label: 'Baja',    color: 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400',        dot: 'bg-zinc-400' },
  MEDIA:   { label: 'Media',   color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300',     dot: 'bg-blue-500' },
...
function ExpedienteCombobox({ value, onChange }: {
  ...
  useEffect(() => { if (value) setInputText(value.label) }, [value])
    ...
    <div ref={containerRef} className="relative flex items-center gap-2">
      ...
      {open && !value && results.length > 0 && (
        <div className="absolute top-full left-0 z-20 mt-1 w-full rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 shadow-lg max-h-48 overflow-y-auto">
          {results.map(exp => (
            <button key={exp.id} type="button" onMouseDown={e => e.preventDefault()}
              onClick={() => {
                const label = exp.caratula || exp.numero || exp.id
                onChange({ id: exp.id, label }); setInputText(label); setOpen(false)
              }}
...
function ComentariosSection({ tareaId }: { tareaId: string }) {
  ...
  useEffect(() => { endRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [comentarios.length])

  async function handleAdd() {
    const t = texto.trim()
    if (!t) return
    try { await addComentario.mutateAsync({ tareaId, contenido: t }); setTexto('') }
    catch { toast.error('No se pudo agregar el comentario') }
  }
  ...
  return (
    <div className="border-t border-zinc-200 dark:border-white/10 pt-4 space-y-3">
      ...
      {isLoading ? (
        <div className="space-y-2">{[0,1].map(i => <div key={i} className="h-10 bg-zinc-100 dark:bg-zinc-800 rounded animate-pulse" />)}</div>
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
          {comentarios.map(c => (
            <div key={c.id} className="flex gap-2 text-sm">
              <div className="shrink-0 h-6 w-6 rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center text-[10px] font-semibold uppercase">
                {c.autor?.nombre?.[0] ?? '?'}
              </div>
      ...
      <div className="flex gap-2">
        <input type="text" value={texto} onChange={e => setTexto(e.target.value)}
          ...
          className="flex-1 rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-800 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        <button type="button" onClick={handleAdd} disabled={!texto.trim() || addComentario.isPending}
...
function TareaEditDialog({ tarea, onClose }: { tarea: TareaWithRelations; onClose: () => void }) {
  ...
  async function handleComplete() {
    try { await completarTarea.mutateAsync(tarea.id); onClose() }
    catch { toast.error('No se pudo completar') }
  }

  async function handleReopen() {
    try { await reopenTarea.mutateAsync(tarea.id); onClose() }
    catch { toast.error('No se pudo reabrir') }
  }
      ...
      <div className="w-full max-w-lg bg-white dark:bg-zinc-900 rounded-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-white/10 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            {completada && <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300">Completada</span>}
          </div>
          <button type="button" onClick={onClose} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200"><X className="h-5 w-5" /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">
              Etiquetas <span className="text-zinc-400 font-normal">(Enter o coma)</span>
            </label>
            <EtiquetasInput value={etiquetas} onChange={setEtiquetas} />
          </div>
          <div className="flex items-center gap-2 pt-1">
            <button type="button" onClick={handleSave} disabled={!dirty || updateTarea.isPending}
            ...
            {!completada ? (
              <button type="button" onClick={handleComplete} disabled={completarTarea.isPending}
              ...
              <button type="button" onClick={handleReopen} disabled={reopenTarea.isPending}
      ...
      <div className="w-full max-w-lg bg-white dark:bg-zinc-900 rounded-2xl shadow-xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100">Cancelar</button>
...
// ---------------------------------------------------------------------------
// TareaCard — kanban card
// ---------------------------------------------------------------------------

function TareaCard({ tarea, onComplete, onMoveToProgress, onClick }: {
  ...
  return (
    <button type="button" onClick={onClick}
      ...
      <div className="flex items-start gap-2">
        <div role="button" tabIndex={-1} onClick={onComplete} onKeyDown={e => e.key === 'Enter' && onComplete(e as any)}
          className={cn('mt-0.5 shrink-0 h-4 w-4 rounded-full border-2 transition-colors flex items-center justify-center',
            completada ? 'border-green-500 bg-green-500' : 'border-zinc-300 dark:border-zinc-600 hover:border-green-400')}>
          {completada && <CheckCircle2 className="h-3 w-3 text-white" />}
        </div>
        <p className={cn('text-sm font-medium leading-snug flex-1 text-zinc-900 dark:text-zinc-100',
          completada && 'line-through text-zinc-400 dark:text-zinc-500')}>
          {tarea.titulo}
        </p>
      </div>
      {tarea.descripcion && !completada && (
        <p className="text-xs text-zinc-500 dark:text-zinc-400 pl-6 leading-snug line-clamp-2">{tarea.descripcion}</p>
      )}
      <div className="flex flex-wrap gap-1.5 pl-6">
        {tarea.expediente ? (
          <Link to={`/expedientes/${tarea.expediente.id}`} onClick={e => e.stopPropagation()}
...
function TareaRow({ tarea, onComplete, onClick }: {
  tarea: TareaWithRelations
  onComplete: (e: React.MouseEvent) => void
  onClick: () => void
}) {
  ...
  return (
    <button type="button" onClick={onClick}
      className={cn(
        'w-full text-left flex items-center gap-3 px-4 py-3 border-b border-zinc-100 dark:border-white/5',
        'hover:bg-zinc-50 dark:hover:bg-white/[0.03] transition-colors',
        completada && 'opacity-50',
      )}>
      <div role="button" tabIndex={-1} onClick={onComplete} onKeyDown={e => e.key === 'Enter' && onComplete(e as any)}
        className={cn('shrink-0 h-4 w-4 rounded-full border-2 transition-colors flex items-center justify-center',
          completada ? 'border-green-500 bg-green-500' : 'border-zinc-300 dark:border-zinc-600 hover:border-green-400')}>
        {completada && <CheckCircle2 className="h-3 w-3 text-white" />}
      </div>
      <div className={cn('shrink-0 w-1 h-7 rounded-full', pConfig.dot)} />
      <div className="flex-1 min-w-0">
        <p className={cn('text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate',
          completada && 'line-through text-zinc-400 dark:text-zinc-500')}>
          {tarea.titulo}
        </p>
        {tarea.descripcion && !completada && (
          <p className="text-xs text-zinc-400 truncate mt-0.5">{tarea.descripcion}</p>
        )}
      </div>
      <div className="hidden sm:flex items-center gap-2 shrink-0">
        {tarea.etiquetas?.slice(0, 2).map(tag => (
          <span key={tag} className="px-1.5 py-0.5 rounded text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400">{tag}</span>
        ))}
        {tarea.expediente ? (
          <Link to={`/expedientes/${tarea.expediente.id}`} onClick={e => e.stopPropagation()}
            className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800 max-w-[120px] truncate hover:bg-amber-100">
            <FolderOpen className="h-2.5 w-2.5 shrink-0" />
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2 pb-1 border-b border-zinc-200 dark:border-white/10">
        ...
        <span className="rounded-full bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-xs font-medium text-zinc-500 dark:text-zinc-400">{count}</span>
      </div>
...
type TipoFiltro = 'todas' | 'libre' | 'expediente'
...
export default function TareasPage() {
  ...
  const tipoFiltro = filtroTipo === 'todas' ? null : filtroTipo as 'libre' | 'expediente'
  ...
  const { data: completadasRes } = useTareas({
    ...sharedFilters, estado: 'COMPLETADA', pageSize: 20, sortBy: 'created_at', sortOrder: 'desc',
  })
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">Tareas</h1>
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg border border-zinc-200 dark:border-white/10 overflow-hidden">
            <button type="button" onClick={() => setVista('tablero')} title="Vista tablero"
              className={cn('p-2 transition-colors', vista === 'tablero'
                ? 'bg-zinc-800 dark:bg-zinc-100 text-white dark:text-zinc-900'
                : 'bg-white dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-white/5')}>
              <Columns3 className="h-4 w-4" />
            </button>
            <button type="button" onClick={() => setVista('lista')} title="Vista lista"
              className={cn('p-2 border-l border-zinc-200 dark:border-white/10 transition-colors', vista === 'lista'
                ? 'bg-zinc-800 dark:bg-zinc-100 text-white dark:text-zinc-900'
                : 'bg-white dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-white/5')}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex rounded-lg border border-zinc-200 dark:border-white/10 overflow-hidden text-xs font-medium">
          <button type="button" onClick={() => setMisTareas(true)}
            className={cn('px-3 py-1.5 transition-colors', misTareas
              ? 'bg-blue-600 text-white'
              : 'bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-white/5')}>
            Mis tareas
          </button>
          <button type="button" onClick={() => { setMisTareas(false); setFiltroAsignado('') }}
            className={cn('px-3 py-1.5 border-l border-zinc-200 dark:border-white/10 transition-colors', !misTareas
              ? 'bg-blue-600 text-white'
              : 'bg-white dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-white/5')}>
            Todo el equipo
          </button>
      ...
      {/* Content */}
      {loading ? (
        <div className={cn('grid gap-5', vista === 'tablero' ? 'grid-cols-1 md:grid-cols-3' : 'grid-cols-1')}>
          {Array.from({ length: vista === 'tablero' ? 3 : 1 }).map((_, i) => (
            <div key={i} className="space-y-2">
              <div className="h-4 w-24 bg-zinc-100 dark:bg-zinc-800 rounded animate-pulse" />
              {[0,1,2].map(j => <div key={j} className="h-20 bg-zinc-100 dark:bg-zinc-800 rounded-xl animate-pulse" />)}
            </div>
          ))}
        </div>
      ) : vista === 'tablero' ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <Column title="Pendiente" count={pendientes.length} accentClass="text-amber-600 dark:text-amber-400">
            {pendientes.length === 0
              ...
              : pendientes.map(t => (
                <TareaCard key={t.id} tarea={t} onClick={() => setEditTarea(t)}
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-white/10 overflow-hidden bg-white dark:bg-zinc-900/80">
          {pendientes.length > 0 && (
            <>
              <div className="px-4 py-2 bg-zinc-50 dark:bg-white/[0.03] border-b border-zinc-100 dark:border-white/5">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400">
                  Pendiente <span className="ml-1 text-zinc-400 font-normal">({pendientes.length})</span>
                </span>
              </div>
              {pendientes.map(t => <TareaRow key={t.id} tarea={t} onClick={() => setEditTarea(t)} onComplete={e => handleComplete(t, e)} />)}
            </>
          )}
          {enProgreso.length > 0 && (
            <>
              <div className="px-4 py-2 bg-zinc-50 dark:bg-white/[0.03] border-b border-zinc-100 dark:border-white/5">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400">
                  En progreso <span className="ml-1 text-zinc-400 font-normal">({enProgreso.length})</span>
                </span>
              </div>
              {enProgreso.map(t => <TareaRow key={t.id} tarea={t} onClick={() => setEditTarea(t)} onComplete={e => handleComplete(t, e)} />)}
            </>
          )}
          {completadas.length > 0 && (
            <>
              <div className="px-4 py-2 bg-zinc-50 dark:bg-white/[0.03] border-b border-zinc-100 dark:border-white/5">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-green-600 dark:text-green-400">
                  Completadas (últimas 20) <span className="ml-1 text-zinc-400 font-normal">({completadas.length})</span>
                </span>
              </div>
              {completadas.map(t => <TareaRow key={t.id} tarea={t} onClick={() => setEditTarea(t)} onComplete={e => e.stopPropagation()} />)}
            </>
          )}
          {pendientes.length === 0 && enProgreso.length === 0 && completadas.length === 0 && (
            <p className="px-4 py-8 text-center text-sm text-zinc-400 italic">No hay tareas con los filtros actuales.</p>
          )}
        </div>
      )}
