...
import {
  Gavel, Pin, PinOff, Plus, Loader2, X, FileText, Search, ExternalLink,
} from 'lucide-react'
import { Card } from './detail-helpers'
import { EmptyState } from '@/components/shared/empty-state'
import { ConfirmDialog } from '@/components/shared/confirm-dialog'
import {
  useJurisprudenciaList,
  useExpedienteJurisprudencia,
  useFijarJurisprudencia,
  useDesfijarJurisprudencia,
  type JurisprudenciaDocumento,
  type ExpedienteJurisprudenciaRow,
} from '@/hooks/use-jurisprudencia'
import { toast } from '@/stores/toast-store'

interface Props { expedienteId: string }

// ─── Dialog: fijar fallo desde el corpus ──────────────────────────────────
...
function FijarDialog({
  ...
  const { data: corpus = [], isLoading } = useJurisprudenciaList()
  const fijar = useFijarJurisprudencia()
  const [search, setSearch] = useState('')
  const [selected, setSelected] = useState<JurisprudenciaDocumento | null>(null)
  ...
  const submit = () => {
    ...
    fijar.mutate(
      ...
      {
        onSuccess: () => { toast.success('Fallo fijado'); onClose() },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg rounded-xl border border-white/10 bg-white dark:bg-zinc-900/80 shadow-xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="px-5 py-3 border-b border-white/5">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Buscar en tu corpus de fallos…"
                  autoFocus
              ...
              <p className="mt-2 text-[10px] text-zinc-500 dark:text-zinc-400">
                Solo aparecen fallos <strong>indexados</strong> y no fijados. Los fallos fijados van como contexto preferente al redactar escritos.
              </p>
            </div>
              ...
              {isLoading ? (
                ...
                <div className="py-8 text-center">
                  <p className="text-sm text-zinc-400">
                    {corpus.length === 0
                      ? 'Tu corpus de fallos está vacío.'
                      : search.trim()
                      ? 'Ningún fallo coincide con tu búsqueda.'
                      : 'No hay más fallos disponibles para fijar.'}
                  </p>
                  <Link to="/jurisprudencia" className="mt-3 inline-flex items-center gap-1 text-xs text-violet-300 hover:underline">
                    Ir a Jurisprudencia <ExternalLink className="h-3 w-3" />
                  </Link>
                </div>
              ) : (
                  ...
                  {disponibles.map(doc => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-medium text-zinc-900 dark:text-zinc-100 truncate">{doc.caratula}</p>
                        <p className="text-[10px] text-zinc-500 dark:text-zinc-400">
                          {doc.tribunal && <>{doc.tribunal}</>}
          ...
          <>
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              <div className="rounded-lg border border-violet-500/30 bg-violet-950/30 p-3">
                ...
                <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{selected.caratula}</p>
                <p className="text-[10px] text-zinc-400 mt-0.5">
                  {selected.tribunal && <>{selected.tribunal}</>}
                  {selected.fecha && <> · {new Date(selected.fecha).toLocaleDateString('es-AR')}</>}
                  {selected.jurisdiccion && <> · {selected.jurisdiccion}</>}
                </p>
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Por qué fijás este fallo a este expediente"
...
export function TabJurisprudencia({ expedienteId }: Props) {
  const { data: rows = [], isLoading } = useExpedienteJurisprudencia(expedienteId)
  const desfijar = useDesfijarJurisprudencia()
  const [openFijar, setOpenFijar] = useState(false)
  const [confirmDesfijar, setConfirmDesfijar] = useState<ExpedienteJurisprudenciaRow | null>(null)
    ...
    <>
      ...
      >
        {isLoading ? (
          ...
          <EmptyState
            icon={Gavel}
            title="Sin fallos fijados"
            description="Fijá los precedentes relevantes a este expediente. Cuando generes un escrito, esos fallos entran al contexto del modelo como referencias preferentes."
            actionLabel="Fijar primer fallo"
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="shrink-0 rounded-full bg-violet-500/15 px-2 py-0.5 text-[9px] font-medium uppercase tracking-wider text-violet-300">
                      {row.documento.tipo.replace('_', ' ')}
                    </span>
                  </div>
                  <p className="text-[10px] text-zinc-500 dark:text-zinc-400 mt-0.5">
                    {row.documento.tribunal && <>{row.documento.tribunal} · </>}
                    {row.documento.fecha && <>{new Date(row.documento.fecha).toLocaleDateString('es-AR')} · </>}
        ...
        <p className="mt-3 text-[10px] text-zinc-600 dark:text-zinc-300">
          Además de estos fallos, al redactar el agente hace retrieval semántico sobre todo tu corpus de jurisprudencia buscando lo más afín.
        </p>
      </Card>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          desfijar.mutate(
            ...
            {
              onSuccess: () => { toast.success('Fallo desfijado'); setConfirmDesfijar(null) },
              onError: (err) => toast.error(err.message),
            }
          )
        }}
        title="Desfijar fallo"
        description={`¿Desfijar "${confirmDesfijar?.documento.caratula}" de este expediente? El fallo sigue en tu corpus pero deja de ir como contexto en los escritos.`}
