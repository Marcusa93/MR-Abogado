-- ============================================================
-- Marco Rossi Estudio Jurídico - Migración 009: Correcciones de Calidad
--
-- Fix 1: Partial UNIQUE en clientes.dni para soft-delete
-- Fix 2: Prevenir doble escritura en audit_log (trigger + RPC)
-- ============================================================

-- ============================================================
-- FIX 1: clientes.dni UNIQUE conflicta con soft-delete
--
-- Problema: El UNIQUE absoluto en dni/cuil impide reutilizar
-- un DNI/CUIL de un cliente eliminado (soft-deleted).
-- Si un cliente se borra y luego se reingresa, viola la
-- constraint UNIQUE.
--
-- Solución: Reemplazar UNIQUE constraint por un partial
-- unique index que solo aplique a registros activos
-- (deleted_at IS NULL).
-- Nota: 00001 ya define estos índices parciales directamente,
-- por lo que usamos IF NOT EXISTS para idempotencia.
-- ============================================================

-- Eliminar constraint UNIQUE original de dni si existe
ALTER TABLE public.clientes DROP CONSTRAINT IF EXISTS clientes_dni_key;

-- Crear partial unique index: solo registros activos
CREATE UNIQUE INDEX IF NOT EXISTS idx_clientes_dni_active
  ON public.clientes (dni)
  WHERE deleted_at IS NULL;

-- Hacer lo mismo para cuil
ALTER TABLE public.clientes DROP CONSTRAINT IF EXISTS clientes_cuil_key;

CREATE UNIQUE INDEX IF NOT EXISTS idx_clientes_cuil_active
  ON public.clientes (cuil)
  WHERE deleted_at IS NULL AND cuil IS NOT NULL;

-- ============================================================
-- FIX 2: Doble escritura en audit_log
--
-- Problema: Las RPCs cambiar_estado_expediente y otras
-- hacen UPDATE a expedientes Y también INSERT manualmente
-- en audit_log. Pero el trigger audit_expedientes_changes
-- también dispara y escribe OTRA fila en audit_log.
-- Resultado: 2 filas de audit por cada operación RPC.
--
-- Solución: Token de sesión HMAC-like que solo las funciones
-- SECURITY DEFINER conocen. El trigger revisa el token y
-- se salta si ya hay audit manual.
-- SET LOCAL se resetea automáticamente al fin de la tx.
-- ============================================================

-- Helper: genera el token de skip confiable para la transacción actual
...
-- Revocar ejecución directa del helper
...
-- Recrear trigger de auditoría de expedientes con verificación de token
...
-- Recrear cambiar_estado_expediente con SET LOCAL antes del UPDATE
...
BEGIN
  ...
  -- Validar transición (terminal states have empty arrays)
